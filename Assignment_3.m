...
testing=[K1(split:m1,1:5);K2(split:m2,1:5)];
...
label=predict(tree,testing);
accur=[accur,metrics(label,testLabels)];
Mdl = fitcsvm(training,trainLabels);
label = predict(Mdl,testing); %assignment 3
accur=[accur,metrics(label,testLabels)];
